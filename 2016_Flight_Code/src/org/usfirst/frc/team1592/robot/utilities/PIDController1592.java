 ...
 */
public class PIDController1592 implements PIDInterface, LiveWindowSendable, Controller1592
{
	...
	protected void calculate()
	{
		...
		if(enabled)
		{
			...
			synchronized(this)
			{
				...
				if(m_pidInput.getPIDSourceType()
					.equals(PIDSourceType.kRate))
				{
					if(m_P != 0)
					{
				...
				{
					//Edited by D. Dyer to prevent including integral gain when saturated by proportional gain
					if(m_I != 0)
					{
						...
						if (Math.abs(potentialIOutput) < m_IZone) {
							...
							if(potentialOutput < m_maximumOutput)
							{
	 ...
	 *$
	 * @param max output from integral contribution
	 */
